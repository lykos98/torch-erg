...
def load_pglib_opf(path):
    ...
    bus_data.loc[(bus_data['Gs'] != 0) | (bus_data['Bs'] !=0), 'bustype'] = 3
    bus_data.loc[bus_data['bus_i'].isin(genlist), 'bustype'] = 1
...
def infer_link_type(bus_data, dfnetwork_T):
    ...
    dfnetwork['to_type'] = 2
    genlist = bus_data.loc[bus_data['bustype'] == 1, 'bus_i'].tolist()
    shuntlist = bus_data.loc[bus_data['bustype'] == 3, 'bus_i'].tolist()
    
    dfnetwork.loc[dfnetwork['fbus'].isin(shuntlist), 'from_type'] = 3
    dfnetwork.loc[dfnetwork['fbus'].isin(genlist), 'from_type'] = 1
    
    dfnetwork.loc[dfnetwork['tbus'].isin(genlist), 'to_type'] = 1
    dfnetwork.loc[dfnetwork['tbus'].isin(shuntlist), 'to_type'] = 3
    dfnetwork['link_type'] = dfnetwork['from_type'] + dfnetwork['to_type']
    dfnetwork.loc[(dfnetwork['from_type'] == 1) & (dfnetwork['to_type'] == 1), 'link_type'] = 1
    dfnetwork.loc[(dfnetwork['from_type'] == 1) & (dfnetwork['to_type'] == 2), 'link_type'] = 2
    dfnetwork.loc[(dfnetwork['from_type'] == 2) & (dfnetwork['to_type'] == 1), 'link_type'] = 2
    dfnetwork.loc[(dfnetwork['from_type'] == 1) & (dfnetwork['to_type'] == 3), 'link_type'] = 3
    dfnetwork.loc[(dfnetwork['from_type'] == 3) & (dfnetwork['to_type'] == 1), 'link_type'] = 3
    return(dfnetwork)
...
def pow_parser(name):
    ...
    k1,k2,k3,q1,q2,q3 = hc.avg_degreetype(torch.tensor(truemat),buslist)
